...
class QuantumClientManager:
    ...
    def on_connection_status(self, status):
        ...
        if status == 1:  # Connected
            self.connected = True
            self.reconnecting = False  # 重置重连标志
            self.connection_event.set()
            self.error = None  # 清除错误状态
            ...
            if status == 3 or status == 4:
                ...
                self.error = "Connection disconnected"
            
            # 当连接断开时，立即通知所有等待的任务失败
            with self.lock:
                # 通知所有挂起的任务连接已断开
                for job_id, event in list(self.pending_jobs.items()):
                    logger.info(f"Notifying job {job_id} about connection loss")
                    # 设置事件，使等待的线程能够退出
                    event.set()
                    
                    # 为对应的API实例设置连接丢失的错误状态
                    if job_id in self.job_to_api_map:
                        api_instance = self.job_to_api_map[job_id]
                        if hasattr(api_instance, 'start_time_dt') and api_instance.start_time_dt:
        ...
        for attempt in range(max_retries):
            try:
                ...
                with self.lock:
                    ...
                    logger.info("Re-initialized quantum client")
                    ...
                    if result != 0:
                        ...
                        raise Exception(f"Failed to reconnect with error code: {result}, last error: {error_code}")
                    
                    # 等待连接确认
                    if not self.connection_event.wait(timeout=10.0):
                        raise Exception("Timeout waiting for reconnection")
                        
                    if self.error and "disconnected" in self.error.lower():
                        self.error = None  # 清除断开连接的错误状态
                        
                logger.info("Successfully reconnected to quantum service")
                self.connected = True
                self.reconnecting = False  # 重置重连标志
                return True
                ...
                if attempt < max_retries - 1:
                    ...
                    self.error = f"Reconnection failed after {max_retries} attempts: {e}"
                    self.reconnecting = False  # 重置重连标志
                    return False
   ...
   # 结果处理回调
    ...
    def send_model_call(self, payload, command_type=b"", api_instance=None, blobs=None):
        ...
        if not self.wait_for_connection():
            logger.error("Cannot send command: connection not ready after waiting")
            return None
        ...
        for attempt in range(max_retries):
            with self.lock:
                if not self.connected or self.client_handle is None:
                    logger.warning(f"Connection not established, attempting to initialize (attempt {attempt + 1}/{max_retries})...")
                    if not self.initialize():  # 尝试初始化/重连
                        if attempt == max_retries - 1:
        ...
        with self.lock:
            if not self.connected or self.client_handle is None:
                logger.error("Cannot send command: connection not established")
                return None
            ...
            if not self.connected:
                logger.error("Connection lost before sending command")
                quantum_free_ref(data_container)
                quantum_free_ref(input_data)
                return None
    ...
    def query_result(self, job_id, timeout=180):
        ...
        # 等待连接就绪
        if not self.wait_for_connection():
            logger.error(f"Cannot query result for job {job_id}: connection not ready after waiting")
            # 为API实例设置结束时间
            if job_id in self.job_to_api_map:
                api_instance = self.job_to_api_map[job_id]
                if hasattr(api_instance, 'start_time_dt') and api_instance.start_time_dt:
            ...
            return {
                "status": "failed",
                "data": "Connection not ready for query",
                "error_reason": "Connection not ready for query",
                "job_id": job_id
            }
        ...
        while time.time() - start_time < timeout:
            # 检查连接状态，如果断开则立即返回错误
            if not self.connected:
                logger.error(f"Connection lost before querying result for job {job_id}")
                # 为API实例设置结束时间
                if job_id in self.job_to_api_map:
                    api_instance = self.job_to_api_map[job_id]
                    if hasattr(api_instance, 'start_time_dt') and api_instance.start_time_dt:
                ...
                return {
                    "status": "failed",
                    "data": "Connection lost during query",
                    "error_reason": "Connection lost during query",
                    "job_id": job_id
                }
            ...
            # 使用较短的超时时间来定期检查连接状态
            check_interval = 1.0  # 每秒检查一次连接状态
            wait_start = time.time()
            
            while time.time() - wait_start < timeout and time.time() - start_time < timeout:
                # 再次检查连接状态
                if not self.connected:
                    logger.error(f"Connection lost during query for job {job_id}")
                    # 清理等待状态
                ...
                # 等待事件，使用较短的超时时间以便检查连接状态
                remaining_time = min(check_interval, timeout - (time.time() - start_time))
                if event.wait(timeout=remaining_time):
                    # 从队列中获取结果
                    try:
    ...
    def wait_for_connection(self, timeout=30):
        ...
        start_time = time.time()
        while time.time() - start_time < timeout:
            if self.connected and not getattr(self, 'reconnecting', False):
