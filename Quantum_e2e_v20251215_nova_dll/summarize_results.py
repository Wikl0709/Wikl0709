...
SCENE_PASS_MAP = {
    ...
    "file search": ("Result", None, "response_time", "first_token_time"),
    "kbqa": ("Result", None, "response_time", "first_token_time"),
    # "tool calling": ("pass", None, "response_time", "first_token_time"),
    # "memory": ("Result", None, "response_time", "first_token_time"),
...
def scan_one_excel(file_path: Path):
    """扫一个 Excel，返回 {scene: {total:int, pass:int, latency:list, TTFT:list}}"""
    try:
    ...
    scene_stat = defaultdict(lambda: {"total": 0, "pass": 0, "latency": [], "TTFT": []})
    for _, row in df.iterrows():
        scene = str(row.get("scene", "")).strip().lower()
        if not scene or scene not in SCENE_PASS_MAP:
            continue

        pass_col, _, latency_col, ttft_col = SCENE_PASS_MAP[scene]
...
def summarize(excel_paths: list[Path], output_path: Path = Path("summary.csv")):
    """多文件聚合汇总"""
    final = defaultdict(lambda: {"total": 0, "pass": 0, "latency": [], "TTFT": []})
