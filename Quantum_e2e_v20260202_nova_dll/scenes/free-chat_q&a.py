...
def create_evaluation_prompt(question, result):
    prompt = f"""
This is a conversation between a user and an AI Agent.
From a user experience perspective, evaluate whether the AI Agent's response
in this turn is acceptable and correct, based on the rules below.

[Evaluation Rules]
1. Whether the AI Agent's response is relevant to the user's input.
2. Whether the response contains any obvious factual errors.
3. Ignore any JSON formatting issues in the AI Agent's response.

[Important Requirements]
- Output JSON only. Do not include any other text, explanation, or markers.
...
[Data to Evaluate]
User Query: {question} (user input)
AI Agent: {result} (AI Agent response)
