...
class MultimodalAPI:
    ...
    def process_memory_data(self, min_wait_time=1.0, max_wait_time=10.0, enable_random_wait=True):
        ...
        if need_rerun_indices:
            ...
            # 重跑错误的case
            consecutive_failed_count = 0  # 连续失败计数
            rerun_idx = 0
...
if __name__ == "__main__":
    ...
    try:
        ...
        if success:
            ...
            # 搜集Qira日志
            packageLog.main(Path(args.output_dir))


            # # 进行Quantum Core日志注册分析
            # if args.process_document or args.process_all:
            #     logger.info("进行Quantum Core 日志注册分析")
            #     quantum_core_path = get_quantum_core_path()
            #     try:
            ...
            #         if output_files:
            #             logger.info("开始自动判断...")
            #             Auto_Judge.run(args.doc_dir, args.output_dir,output_files, filter_local_brain=args.filter_local_brain)
            #         else:
            #             # 如果没有output_files，则使用默认行为（查找最新文件）
            #             logger.info("没有可用的输出文件，将使用默认行为（查找最新文件）")
            #             Auto_Judge.run(args.doc_dir, args.output_dir, afilter_local_brain=args.filter_local_brain)
            #     else:
