...
# RUN_ID = f"{int(time.time() * 1000)}_{os.getpid()}_{random.randint(1000, 9999)}"
RUN_ID = "Quantum_e2e_TestResult"
...
# DOCUMENT_DIR = OUTPUT_ROOT / "document"
...
# 初始化日志系统（日志写入 OUTPUT_ROOT/logs 下）
...
if __name__ == "__main__":
    ...
    parser.add_argument('--version_config', type=str, default="v20260226", help='生成config.json时使用的version字段')
    ...
    try:
        ...
        try:
            ...
            if zip_candidates:
                # 若打包结果已经是目标文件名，则无需重命名；若存在多个zip，优先选取“非目标名且最新”的zip
                non_target_candidates = [p for p in zip_candidates if p.resolve() != target_zip.resolve()]
    ...
    ####################################################################################
    #日志中获取memory和kbqa的Retrieval结果
    ####################################################################################
    # logger.info("处理在日志中获取memory和kbqa的Retrieval结果")
    # # quantum_core_path = get_quantum_core_path()
    # quantum_core_path = r"D:\AAA\result\R72\PackageAllLogs\pod7_10_28"
    # output_files = [r"D:\AAA\result\R72\testresult\test_results_Memory_KBQA-30-1210-ocrlayout_filesearchv20250128_20260128_153505_cycle1.xlsx"]
