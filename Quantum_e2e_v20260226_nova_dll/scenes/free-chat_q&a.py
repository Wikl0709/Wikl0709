...
Evaluate whether the AI Agent's response is acceptable and correct
STRICTLY based on the rules below.

[Evaluation Scope]
- Only use the rules below to make the judgment.
- Do NOT add any criteria beyond these rules.
- If correctness cannot be determined, assume the response is correct
  unless it clearly violates a rule.

[Evaluation Rules]
1. The response must be relevant to the user's input.
2. The response must not contain clear or obvious factual errors.
3. Ignore any JSON formatting issues in the AI Agent's response.

[Identity Rules]
4. If the user asks the AI's name or identity (e.g., "who are you", "what's your name"),
   the response is correct ONLY if it identifies itself as "Qira".
   Any other name is incorrect.

[Device Rules]
5. If the user's query does NOT mention a device:

   The response is incorrect for device reasons ONLY if it EXPLICITLY treats
   a device as belonging to the user.

   This requires at least one of the following explicit triggers:
   - second-person ownership or context: "your", "you are using", "on your device",
     "on your laptop/phone", "based on your device"
   - explicit claims of detecting or knowing the user's device:
     "I see your device", "I detected your device", "your device model is"
   - presenting a specific device model/ID as if it were user data

   Do NOT infer user-specific context from examples.

6. Any device mentioned as part of capability descriptions or examples
   (e.g., using "e.g.", "for example", "such as", "including", or appearing in parentheses)
   MUST be treated as a generic example and MUST be judged as acceptable.



[Output Constraints - MUST FOLLOW EXACTLY]
- Output MUST be a single valid JSON object and NOTHING ELSE.
- Do NOT include explanations, prefixes, suffixes, or markdown.
...
{{
  ...
  "explanation": "Brief explanation based strictly on the rules",
  "correct_answer": ""
}}

[Additional Output Rules]
- If "is_correct" is true, "correct_answer" MUST be an empty string.
- If "is_correct" is false, "correct_answer" MUST contain the correct response.

[Data to Evaluate]
User Query: {question}
AI Agent Response: {result}
...
if __name__ == "__main__":
    single_input = {
        'question': 'hi',
        'result': 'Hello! How can I assist you today? If you have any questions about your Lenovo XiaoXinPro or need help with tasks on this Wednesday the4th of February,2026 at21:18, feel free to ask.'
    }
