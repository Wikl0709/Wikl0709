...
class MultimodalAPI:
    ...
    def get_response_content_detailed(self):
        ...
        try:
            ...
            # 添加超时设置
            response = requests.get(url, params=params, stream=True,timeout=3600)  # 连接超时30秒，读取超时5分钟
    ...
    #                 byte_array = [(b if b < 128 else b - 256) for b in f.read()]
    #             binary_data.append({
    #                 "mime": mime_type,
    #                 "data": byte_array
    #             })
    ...
    def get_response_content(self):
        ...
        self.text_response_count = 0 
        self.first_text_response_time = None
        self.current_mode = None  # 新增：用于存储识别到的模式
        # 只保存关键的JSON数据
        key_json_data = []
        try:
                    ...
                    if decoded_line.startswith('data: '):
                        ...
                        try:
                            ...
                            if data_json.get("type") == "output-data":
                                ...
                                # >>> 新增：模式识别逻辑 <<<
                                # 在第一个in_progress响应中检测模式
                                if self.current_mode is None and output_data.get("status") == "in_progress":
                                    # 检查是否是handler类型响应
                                    if text_type == "handler":
                                        handler_name = None
                                        if isinstance(text_content, dict):
                                            handler_name = text_content.get("response")
                                        elif isinstance(text_content, str):
                                            try:
                                                text_json = json.loads(text_content)
                                                if isinstance(text_json, dict):
                                                    handler_name = text_json.get("response")
                                            except json.JSONDecodeError:
                                                pass
                                        
                                        if handler_name == "Azure Agent":
                                            self.current_mode = "azure_agent"
                                            logger.info("检测到 Azure Agent 模式")
                                        else:
                                            self.current_mode = "local_brain"
                                            logger.info("检测到 Local Brain 模式")
                                    
                                    # 如果没有明确的handler类型，但包含"Azure Agent"字样
                                    elif isinstance(text_content, str) and "Azure Agent" in text_content:
                                        self.current_mode = "azure_agent"
                                        logger.info("检测到 Azure Agent 模式")
                                    else:
                                        self.current_mode = "local_brain"
                                ...
                                # 确保模式已识别
                                if self.current_mode is None and output_data.get("status") == "complete":
                                ...
                                if output_data.get("status") == "in_progress" and text_type == "text":
                                    ...
                                    if self.current_mode == "local_brain":
                                        token_count_start = 2  # Local Brain模式从第二个text开始
                                    
                                    if self.text_response_count >= token_count_start:
                                        token_count += 1
...
if __name__ == "__main__":
    ...
    parser.add_argument('--excel_dir', type=str, default=r"C:\Users\obe\Desktop\LRIA_llm_eval\eval\Quantum_e2e_v20251209_nova\dataset1126\Memory_question _v3_1016+EN_DOC_30_1031+en_query30_1031_all.xlsx", help='Excel文件目录路径')
    parser.add_argument('--image_dir', type=str, default=r"C:\Users\obe\Desktop\LRIA_llm_eval\eval\Quantum_e2e_v20251209_nova\Image", help='是否需要用到pathlist')
    parser.add_argument('--doc_dir', type=str, default=r"C:\Users\obe\Desktop\LRIA_llm_eval\eval\Quantum_e2e_v20251209_nova\files1114\doc_summary", help='文档目录路径')
    parser.add_argument('--memory_excel', type=str, default=r"C:\Users\obe\Desktop\LRIA_llm_eval\eval\Quantum_e2e_v20251209_nova\Memory_data+_v3_1016_a.xlsx", help='Memory Excel文件路径')
    ...
    # output_files= ['c:\\Users\\obe\\Desktop\\LRIA_llm_eval\\eval\\Quantum_e2e_v20251204_nova\\testresult\\test_results_Memory_question _v3_1016+EN_DOC_30_1031+en_query30_1031_20251208_095533_cycle1.xlsx']
    # # # 使用 output_files 列表调用 Auto_Judge
    # if  output_files:
    #     Auto_Judge.run(args.doc_dir, output_files)
    # else:
    #     # 如果没有output_files，则使用默认行为（查找最新文件）
