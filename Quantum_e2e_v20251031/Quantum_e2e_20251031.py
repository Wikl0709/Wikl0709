...
class MultimodalAPI:
    ...
    def get_response_content(self):
        ...
        try:
            ...
            #                         try:
            #                             text_json = json.loads(text_content)
            #                             result_text = text_json.get("response", text_content)
            #                         except json.JSONDecodeError:
            #                             result_text = text_content
    ...
    # 在process_document_registration_only方法中添加起始时间记录
    def process_document_registration_only(self):
        ...
        # 检查是否需要注册文档
        if not self.is_document_registered(doc_file_list):
            # 记录文档注册开始时间
...
if __name__ == "__main__":
    ...
    parser.add_argument('--excel_dir', type=str, default=r"C:\Users\Lenovo Yanglei AFR\Desktop\LRIA_llm_eval\eval\20251021\bvt\dataset", help='Excel文件目录路径')
    parser.add_argument('--image_dir', type=str, default=r"C:\Users\Lenovo Yanglei AFR\Desktop\LRIA_llm_eval\eval\20251021\bvt\images", help='是否需要用到附件')
    parser.add_argument('--doc_dir', type=str, default=r"C:\Users\Lenovo Yanglei AFR\Desktop\LRIA_llm_eval\eval\20251021\bvt\files", help='文档目录路径')
    parser.add_argument('--memory_excel', type=str, default=r"C:\Users\Lenovo Yanglei AFR\Desktop\LRIA_llm_eval\eval\20251021\bvt\Memory_data _v3_1016.xlsx", help='Memory Excel文件路径')
    ...
    parser.add_argument('--gpu_type', type=str, default="dGPU", choices=["dGPU", "iGPU", "aGPU"], help='GPU类型')
    ...
    #     logger.info(f"找到 {len(excel_files)} 个Excel文件")
        ...
        try:
            ...
            # 停止资源监控并处理数据
            base_name = os.path.splitext(OUTPUT_EXCEL_PATH)[0]
            cycle_output_path = f"{base_name}_cycle{CYCLE_COUNT}.xlsx"
            if monitor:
                stop_resource_monitoring(monitor)
                # 将资源监控数据添加到结果Excel文件中
