...
SYSTEM_PROMPT = """
You evaluate an AI Agent that controls apps and device settings (open/close apps; adjust volume/brightness; enable/disable Do Not Disturb; other settings).
Inputs: the user question and the final answer shown to the user.
Goal: Decide if the requested action(s) were effectively accomplished, judging primarily by what the final answer explicitly states or clearly implies.

Scoring:
- Task_Success_Rate = 1 (Success)
- Task_Success_Rate = 0 (Failure)

Mark Success (1) when the final answer:
- Confirms completion or a matching state (including “already” states).
- Clearly aligns with the intended domain/action (apps, volume/brightness, DND, specific settings).
- Achieves the overall goal even if some specifics differ.(e.g., “Run PowerShell as administrator.” → “Already opened PowerShell for you.” counts as success).
- When direct execution is restricted by OS/permissions, opens the specific, relevant settings page or official flow that enables the user to finish the task (e.g., opens personalization/wallpaper settings).
- Special case — Eye-care mode: For requests about Eye Care/Eye Protection mode, if the final answer states it is “not support/unsupported” due to Lenovo Vantage dependency and Vantage not being installed, count as Success.

Mark Failure (0) when the final answer:
- States inability/denial only, or provides instructions/questions without confirming completion, or is in-progress.
- Explicitly performs the opposite (open↔close, on↔off, start↔stop) or mismatches the domain/target.
- Is too vague to establish the achieved state/goal.

Multi-action requests:
- Prefer success if the primary/overall goal is clearly achieved; minor secondary steps may be missing without causing failure.
...
Evaluate an AI Agent interaction for app control and device settings (open/close apps; volume/brightness; Do Not Disturb; other settings).
Use the question to infer intent, but judge success primarily by what the final answer claims.
Count as success if the overall goal is achieved (even with minor specifics differing) or if a relevant settings page/flow is opened due to permission limits.
Special case: For Eye Care/Eye Protection mode, “not support/unsupported” due to Lenovo Vantage not being installed counts as success.

# User question:
{question}

# Final answer shown to user ("final answer"):
{result}
...
def main(row_dict: Dict[str, Any],
    ...
    score = main(row)
    """
    if "question" not in row_dict or "result" not in row_dict:
        raise ValueError("缺少必需键：'question' 或 'result'")

    question = str(row_dict["question"])
    result   = str(row_dict["result"])  # final answer

    user_prompt = USER_PROMPT_TEMPLATE.format(question=question, result=result)
    messages = [
...
if __name__ == "__main__":
    # 演示：构造一个行dict并评分
    demo_row = {
        "question": "Please enable eye care mode.",
        "result": "eye care mode is not supported"
    }
