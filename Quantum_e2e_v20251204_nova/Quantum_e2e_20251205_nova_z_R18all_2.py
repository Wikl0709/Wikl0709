...
#             if resource_csv_path and os.path.exists(resource_csv_path):
#                 # 文档注册的结果文件路径
#                 doc_reg_result_path = os.path.join(TEST_RESULTS_DIR, f"document_registration_result_{start_timestamp}.xlsx")
#                 try:
#                     add_resource_data_to_results(resource_csv_path, doc_reg_result_path)
#                 except Exception as e:
#                     logger.error(f"添加资源数据到文档注册结果失败: {e}")
...
#             monitor = None
#             resource_csv_path = None
#             file_start_timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            
#             try:
#                 # 为每个文件启动独立的资源监控
#                 monitor, resource_csv_path, _ = start_resource_monitoring(GPU_TYPE)
                
#                 OUTPUT_EXCEL_PATH = os.path.join(TEST_RESULTS_DIR, f"test_results_{excel_file.stem}_{file_start_timestamp}.xlsx")
#                 EXCEL_FILE_PATH = str(excel_file)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if success:
            ...
            logger.info("开始自动判断...")
            # Auto_Judge.run(args.output_files)
