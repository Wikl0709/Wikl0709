...
def all_details_check(query, answer, result):
    if_details_in_chunk = askGPT_agent()
    return str(if_details_in_chunk)

def KBQA_gpt4_judge(self,index, rows):
        rouge = Rouge()
        gpt4_key = 'sk-zk2364edbf8b84bbcf966260a2c88b5498f668f3960c772e'
        gpt4_url = 'https://flag.smarttrot.com/v1/chat/completions'
        headers = {'Content-Type': 'application/json', 'Accept': 'application/json',
                   'Authorization': "Bearer " + gpt4_key}
       
        query = rows['question']
        if "GT_answer" in rows:
            answer = rows['GT_answer']
        elif "GT" in rows:
            answer = rows['GT']
        elif "answer" in rows:
            answer = rows['answer']
        result = rows['response_content']
        it = rows['intent_type']
            ...
            try:
                response = requests.post(gpt4_url, json.dumps(params), headers=headers)
                ...
                if http_status == 200:
                    ...
                    if 'No' in text:
                        ...
                        try:
                            response = requests.post(gpt4_url, json.dumps(params1), headers=headers)
                            # print(response.json())
                            http_status = response.status_code
                            if http_status == 200:
                                response_json = response.json()
                                ...
                                if text == '{<Result: pass>}':
                                    # if 'pass' in text:
                                    pdtext = 'pass'
                                elif text == '{<Result: pass*>}':
                                    # elif 'pas*' in text:
                                    pdtext = 'pass*'
                                elif text == '{<Result: fail>}':
                                    # elif 'fail' in text:
                                    pdtext = 'fail'
                                else:
                                    print('123')
                                    pdtext = 'fail'
                                LLM_list.append([idx, pdtext])
                                result_list.append([idx, pdtext])
                        except:
                            print("Getting GPT4 response error!")
                            result_list.append([idx, 'fail'])
                            LLM_list.append([idx, 'fail'])
                            return result_list, LLM_list
                    elif 'Yes' in text:
                        print('拒答')
                        result_list.append([idx, 'fail'])
                        if single_result==0:
                            ...
                            LLM_list.append([idx, 'pass'])
                        return retrieval_details,retrieval_result,result_list, LLM_list
                else:
                    result_list.append([idx, 'fail'])
                    LLM_list.append([idx, 'fail'])
            except:
...
def run(doc_dir):
    ...
    # INPUT_EXCEL_PATH = r"C:\Users\zhoutian2\Downloads\test_results_Memory_question _v3_1016_20251125_192754_cycle1.xlsx" # 输入Excel路径（包含question、answer、response列）
    # OUTPUT_EXCEL_PATH = r"C:\Users\zhoutian2\Downloads\test_results_Memory_question _v3_1016_20251125_192754_cycle1_1126.xlsx"  # 输出Excel路径（新增Score列）
    ...
    for index, rows in df.iterrows():
        if rows['result'] == "Error: Empty response from server":
            ...
            print('1')
            query_type=rows['dimension']
            # text=all_details_check(query,answer,result)
            ...
            # process_excel_data(df, OUTPUT_EXCEL_PATH)
