...
def scan_one_excel(file_path: Path, filter_local_brain="auto"):
    ...
    scene_stat = defaultdict(lambda: {
        ...
        "retrieval_latency_ms": []  # 新增：用于存储 Retrieval_latency_ms
    })
    ...
    for idx, row in df.iterrows():
        ...
        is_error_result = (
            ...
            or str(result).startswith("CTTVError: Task failed -")or str(result).startswith("CTTVError: Stream timeout:")
        )
        ...
        # 收集 Retrieval_latency_ms
        retrieval_latency_ms = None
        if "Retrieval_latency_ms" in row:
            try:
                val = float(row["Retrieval_latency_ms"])
                if pd.notna(val) and val >= 0:
                    retrieval_latency_ms = val
                else:
                    retrieval_latency_ms = None
        ...
        if scene == "memory":
            ...
            # 添加 Retrieval_latency_ms
            if retrieval_latency_ms is not None:
                scene_stat[scene]["retrieval_latency_ms"].append(retrieval_latency_ms)
...
def summarize(excel_paths: list[Path], output_path: Path = Path("summary.csv"), filter_local_brain="auto", dir_path: Path = None):
    ...
    for file_path in excel_paths:
        ...
        for scene, v in sub.items():
            ...
            final[scene]["retrieval_latency_ms"].extend(v["retrieval_latency_ms"])  # 新增
    ...
    for scene, v in final.items():
        ...
        # 计算 Retrieval Latency/ms 平均值
        avg_retrieval_latency = None
        if v["retrieval_latency_ms"] and len(v["retrieval_latency_ms"]) > 0:
            avg_retrieval_latency = sum(v["retrieval_latency_ms"]) / len(v["retrieval_latency_ms"])
        ...
        row = {
            ...
            "Retrieval Latency/ms": avg_retrieval_latency if avg_retrieval_latency is not None else "--",
            "Comments": ""
        }
    ...
    df = pd.DataFrame(records, columns=[
        ...
        "Final Answer", "TTFT", "Completion", "Retrieval Latency/ms", "Comments"
    ])
    ...
    with pd.ExcelWriter(output_path, engine='xlsxwriter') as writer:
        ...
        worksheet.set_column('J:J', 15)  # 新增列
        ...
        worksheet.merge_range('I1:I2', 'Retrieval Latency/ms', subheader_format)  # 新增
        ...
        for row_idx in range(2, len(df) + 2):
            for col_idx in range(0, 10):  # 从 9 -> 10
