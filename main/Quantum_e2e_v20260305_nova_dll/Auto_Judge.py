...
def load_scene_module(scene_name):
    ...
    scene_to_module = {
        ...
        "multi-turn GQA": "general_qa_multi_turn"
    }
...
def _judge_one_row(index, row, doc_dir, out_dir, test_mode):
    ...
    try:
        ...
        # 只允许这些场景
        if scene_key not in ["memory", "kbqa", "file search", "tool calling", "multimodal", "free-chat q&a", "multi-turn gqa"]:
        ...
        # Result 字段
        if scene_key in ["tool calling", "multi-turn GQA"]:
        ...
        # Reason / pass
        if scene_key in ["tool calling", "multimodal", "multi-turn GQA"]:
...
def run(doc_dir, out_dir, excel_files=None, filter_local_brain='no', test_mode='local', MAX_WORKERS=64):
