...
Rules:
- 3 (Fully Successful): The final answer explicitly indicates completion/success of the intended action(s) (e.g., "successfully", "have sent", "completed", "done", "OK"), and does NOT indicate any failure. Merely listing steps, instructions, guidance, or suggestions without stating that the action was executed does NOT count as success.
- 2 (Partially Successful): The final answer indicates some part was completed successfully, but also states failure/limitation for other parts (e.g., "unable", "failed", "not supported" for certain sub-tasks).
- 1 (Totally Failed): The final answer indicates overall failure with no successful part (e.g., apologies like "I'm sorry", inability, instructions-only response, or any text that does not assert completed execution).
Output STRICT JSON:
{
 "Task_Success_Rate": 1|2|3,
...
{result}
Judge Task_Success_Rate strictly from final answer text. Instructions-only or apology ("I'm sorry") without explicit execution counts as failure.
"""
...
def main(row_dict: Dict[str, Any],
    ...
    return {"Task_Success_Rate": tsr, "TSR_Rationale": rationale, "pass": is_pass}
...
if __name__ == "__main__":
    # 演示：构造一个行dict并评分
    demo_row = {
        "question": "帮我把日报发给项目群，并在Excel里把昨天的数据汇总到Sheet2。",
