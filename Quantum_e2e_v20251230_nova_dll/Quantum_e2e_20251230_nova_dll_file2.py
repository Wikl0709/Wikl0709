...
class QuantumClientManager:
    ...
    #     参数:
    #         payload: 要发送的负载数据
    #         command_type: 命令类型 (b"session", b"document", b"query", b"model_call")
    #         api_instance: 与此调用关联的MultimodalAPI实例
    #         blobs: 二进制blob列表，每个blob包含'mime'和'data'字段
    #     """
    #     with self.lock:
    #         if not self.connected or self.client_handle is None:
    #             if not self.initialize():
    ...
    #         logger.info(f"发送 {command_type.decode()} 调用，负载: {json.dumps(payload)}")
    ...
    #                 if not data:
    #                     logger.warning(f"跳过空的blob，mime类型: {mime}")
    #                     continue
    ...
    #                     if blob_ptr:
    #                         blob_ptrs.append(blob_ptr)
    #                         logger.info(f"成功创建blob: mime={mime}, 大小={len(data_bytes)} 字节")
    #                     else:
    #                         logger.error(f"创建blob数据失败，mime类型: {mime}")
                            
    #                 except Exception as e:
    #                     logger.error(f"创建blob数据时出错，mime类型 {mime}: {e}")
    #                     continue
    ...
    #                     None
    #                 )
    #                 logger.info(f"创建了包含 {len(blob_ptrs)} 个blobs的数据容器")
    #             else:
    #                 # 如果所有附件都处理失败，使用空数组
    #                 empty_array = (ctypes.c_void_p * 0)()
    #                 data_container = quantum_create_data_container(
    ...
    #                     None
    #                 )
    #                 logger.warning("没有创建有效的blobs，使用空数据容器")
            
    #         if not data_container:
    #             logger.error("创建数据容器失败")
    #             return None
    ...
    #             session_id_bytes = b""
    #         else:
    #             logger.warning(f"没有有效的会话ID用于 {command_type.decode()} 命令")
    ...
    #         # 发送命令
    #         logger.info("quantum_send_command 开始")
    #         job_id = quantum_send_command(self.client_handle, input_data)
    #         logger.info("quantum_send_command 结束")
    ...
    #         logger.info(f"发送了 {command_type.decode()} 调用，任务ID: {job_id}")
    #         return job_id
...
class MultimodalAPI:
    ...
    def process_memory_data(self, min_wait_time=1.0, max_wait_time=10.0, enable_random_wait=True):
            ...
            if not memory_api.create_session():
                ...
                for col_index, column_name in enumerate(user_content_columns):
                    if column_name in row and (not pd.isna(row[column_name]) and row[column_name] != ""):
                        # 计算写入位置
                        result_col_index = len(df.columns) + col_index * 3  # 每个user_content列对应3列：结果、响应时间、TTFT
        ...
        if response_times:
            ...
            logger.info(f"响应时间平均值: {avg_response_time:.2f}秒 (共 {len(response_times)} 个数据点)")
        else:
            avg_response_time = 0
        ...
        if ttft_times:
            ...
            logger.info(f"TTFT平均值: {avg_ttft:.3f}秒 (共 {len(ttft_times)} 个数据点)")
        else:
            avg_ttft = 0
        ...
        if response_times or ttft_times:
            ...
            worksheet[f'{avg_label_col}{last_row}'] = "平均值"
...
def run_quantum_e2e_process(args):
    ...
    for excel_file in excel_files:
        ...
        try:
            ...
            # 只有在明确要求处理Memory数据时才执行
            if args.process_memory:
                # 检查Memory数据是否已处理（仅针对当前流程）
                if not MultimodalAPI._memory_processed:
                    logger.info("开始处理Memory数据...")
                    memory_api = MultimodalAPI()
                    memory_success = memory_api.process_memory_data_only(args.min_wait_time, args.max_wait_time, args.enable_random_wait)
                    MultimodalAPI._memory_processed = True  # 标记为已处理
                    if memory_success:
                        logger.info("Memory数据处理完成")
                    else:
            ...
            if success and args.process_main:
                success = api.process_main_excel_only(CYCLE_COUNT, args.min_wait_time, args.max_wait_time, args.enable_random_wait)
...
try:
    ...
    quantum_lib = ctypes.CDLL(dll_path)
    logger.info(f"成功加载量子SDK DLL: {dll_path}")
except FileNotFoundError as e:
    logger.error(f"无法找到量子SDK DLL: {e}")
    # 处理错误...
except Exception as e:
    logger.error(f"加载量子SDK DLL时发生错误: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--excel_dir', type=str, default=r"C:\Users\obe\Desktop\LRIA_llm_eval\eval\Quantum_e2e_v20251217_nova_dll\dataset20251215\Memory_KBQA-30-1210-ocr&layout_filesearch_test.xlsx", help='Excel文件目录路径')
    parser.add_argument('--image_dir', type=str, default=r"C:\Users\obe\Desktop\LRIA_llm_eval\eval\Quantum_e2e_v20251217_nova_dll\Image", help='是否需要用到pathlist')
    parser.add_argument('--doc_dir', type=str, default=r"C:\Users\obe\Desktop\LRIA_llm_eval\eval\Quantum_e2e_v20251217_nova_dll\Reg_Documents\KBQA-30-1208-ocr&layout", help='文档注册,文档的路径')
    parser.add_argument('--memory_excel', type=str, default=r"C:\Users\obe\Desktop\LRIA_llm_eval\eval\Quantum_e2e_v20251217_nova_dll\Memory_data _v3_1016_test.xlsx", help='Memory注册,Excel文件路径')
    ...
    parser.add_argument('--min_wait_time', type=float, default=1.0, help='随机等待的最小时间（秒），默认为1.0')
    parser.add_argument('--max_wait_time', type=float, default=10.0, help='随机等待的最大时间（秒），默认为10.0')
    parser.add_argument('--enable_random_wait', type=bool, default=False, help='默认为False,不启用随机等待功能')
        ...
        if hasattr(args, 'test_mode') and args.test_mode == "local":
            ...
            try:
                ...
                if os.path.exists(wifi_control_script):
                    ...
                    # 直接调用脚本关闭WiFi，并捕获输出
                    result = subprocess.run([sys.executable, wifi_control_script, "off"], 
                                          capture_output=True, text=True, check=False)
                    
                    # 检查命令执行结果
                    if result.returncode != 0:
                        logger.error(f"关闭WiFi脚本执行失败，返回码: {result.returncode}")
                    ...
                    if "success" in output or "已 off" in output or "success" in result.stderr.lower():
                        # 等待网络状态稳定
                        time.sleep(60)
                        logger.info("WiFi已成功关闭")
                    else:
                        # 检查是否有失败信息
                        if "所有方法都未能成功offwifi" in output or "failed" in output.lower():
                            ...
                            # 再次检查是否成功关闭
                            # 验证WiFi是否真正关闭
                            verify_result = subprocess.run([sys.executable, wifi_control_script, "on"], 
                                                         capture_output=True, text=True, check=False)
                            verify_output = verify_result.stdout.lower()
                            if "no wireless adapter found" in verify_output or "not found" in verify_output.lower():
                                # 确认WiFi已关闭
                                time.sleep(60)
                                logger.info("WiFi已成功关闭")
                            else:
                                logger.error("WiFi关闭操作可能未成功")
                                logger.error(f"脚本输出: {result.stdout}")
                                logger.error("关闭WiFi失败，无法继续执行，程序退出")
                                sys.exit(1)  # 直接中断运行
                else:
                    logger.error(f"WiFi控制脚本不存在: {wifi_control_script}")
                    logger.error("WiFi控制脚本不存在，无法继续执行，程序退出")
                    sys.exit(1)  # 直接中断运行
            except Exception as e:
            ...
            if hasattr(args, 'test_mode') and args.test_mode == "local":
                ...
                try:
                    ...
                    if os.path.exists(wifi_control_script):
                        ...
                        # 直接调用脚本打开WiFi，并捕获输出
                        result = subprocess.run([sys.executable, wifi_control_script, "on"], 
                                              capture_output=True, text=True, check=False)
                        
                        # 检查命令执行结果
                        if result.returncode != 0:
                            logger.error(f"打开WiFi脚本执行失败，返回码: {result.returncode}")
                            logger.error(f"错误输出: {result.stderr}")
                        ...
                        # 检查输出中是否包含成功信息
                        output = result.stdout.lower()
                        if "success" in output or "已 on" in output or "success" in result.stderr.lower():
                            # 等待网络恢复确保正常
                            time.sleep(60)
                            logger.info("WiFi已成功打开")
                        else:
                            # 检查是否有失败信息
                            if "所有方法都未能成功onwifi" in output or "failed" in output.lower():
                                ...
                                # 验证WiFi是否真正打开
                                verify_result = subprocess.run([sys.executable, wifi_control_script, "off"], 
                                                             capture_output=True, text=True, check=False)
                                verify_output = verify_result.stdout.lower()
                                if "no wireless adapter found" not in verify_output and "not found" not in verify_output.lower():
                                    # 确认WiFi已打开
                                    time.sleep(60)
                                    logger.info("WiFi已成功打开")
                                else:
                                    logger.error("WiFi打开操作可能未成功")
                                    logger.error(f"脚本输出: {result.stdout}")
                                    logger.error("打开WiFi失败，无法继续执行，程序退出")
                                    sys.exit(1)  # 直接中断运行
                    else:
