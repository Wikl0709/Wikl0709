    ...
    for excel_file in excel_files:
        ...
        #         logger.error(f"资源清理过程中出现错误: {e}")
            ...
            try:
                ...
                if resource_csv_path and os.path.exists(resource_csv_path):
                    # 获取执行时间戳
                    execution_timestamp = timestamp# 默认使用当前时间戳
                    ...
                    for cycle_index in range(CYCLE_COUNT):
                        try:
                            # 构造正确的文件路径，使用 execution_timestamp
                            cycle_output_path = OUTPUT_EXCEL_PATH.replace('.xlsx', f'_{execution_timestamp}_cycle{cycle_index+1}.xlsx')
                            
                            # 兼容旧版本路径（如果没有 execution_timestamp）
...
#     return candidate
def get_next_hour_mark(now=None):
    """
    返回严格大于当前时刻的下一个整点（分钟秒为0）。
    示例：
      14:03 -> 15:00
      15:00 -> 16:00
      21:30 -> 22:00
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dll_path', type=str, default=r"C:\Users\XiaoXinPro 16 IAH\Desktop\LRIA_llm_eval\eval\Quantum_e2e_v20251217_nova_dll\quantum-sdk-1.0.8.dll", help='量子SDK DLL文件路径') 
    parser.add_argument('--excel_dir', type=str, default=r"C:\Users\XiaoXinPro 16 IAH\Desktop\LRIA_llm_eval\eval\Quantum_e2e_v20251217_nova_dll\dataset20251215\Brain+Latency+Case+2025-11-13_nova_1124.xlsx", help='Excel文件目录路径')
...
#========================================================================================================================
    ...
    try:
        ...
        while True:
            ...
            # 计算下次执行时间（下一个2小时整点）
            now = datetime.now()
            next_run = get_next_hour_mark(now)
            sleep_seconds = max(1, int((next_run - now).total_seconds()))
            logger.info(
