...
# Prompt：仅判决 TSR
# -----------------------------
SYSTEM_PROMPT = """You are an exacting, detail-oriented evaluator.
You will receive the original user question and the model's final answer (natural-language text shown to the user).
Rules:
- 1 (Successful): The final answer explicitly indicates execution/completion of the requested action(s) AND the action-type and target-domain align with the user's intent in the question. Treat generic domain-aligned verbs (e.g., adjust, reset, change, set, configure) as aligned even if the specific direction/magnitude (e.g., percentage, increase/decrease) is not restated. Direction/magnitude differences alone do NOT cause failure.
- 0 (Failed): The final answer indicates failure/inability OR does not assert executed completion OR the action-type/target-domain do not align (e.g., question asks to open an app but result says it was closed; question asks to lower volume but brightness was adjusted). Only when the final answer explicitly performs the OPPOSITE of the requested direction (open↔close, enable↔disable, start↔stop) should it be considered mismatch.
Examples:
- Q: "Decrease brightness by 50%." A: "I have reset the brightness for you." → 1 (aligned domain; generic verb accepted).
- Q: "Open Photos app." A: "I have closed Photos." → 0 (explicit opposite).
- Q: "Lower volume." A: "I adjusted the brightness." → 0 (domain mismatch).
Output STRICT JSON:
{
 "Task_Success_Rate": 0|1,
 "Rationale": "≤25 words reason, concise, evidence-based"
}
No extra keys, no markdown.
no markdown, no commentary. ONLY this JSON.
Keep each rationale short and specific to the final answer wording.
"""

USER_PROMPT_TEMPLATE = """Evaluate the following example ONLY by the final answer.
# User question:
{question}
# Final answer shown to user ("final answer"):
{result}
Judge Task_Success_Rate strictly from the final answer: explicit execution is required. Assess alignment on action-type and target-domain; accept generic verbs (adjust/reset/change/set/configure). Ignore direction/magnitude differences unless the final answer explicitly performs the opposite (open↔close, enable↔disable, start↔stop). Instructions-only or apology ("I'm sorry") counts as failure.
"""
...
if __name__ == "__main__":
    # 演示：构造一个行dict并评分
    demo_row = {
        "question": "Open Calculator",
        # "result": "已完成：日报已发送到项目群。但Excel操作当前环境不支持。",
        "result": "Already open calculator app for you."
    }
