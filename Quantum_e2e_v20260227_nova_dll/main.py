...
# 导入 combined_analysis 模块
from combined_analysis import parse_log_files, process_excel
# 导入分析注册时长的函数
from analysis_log_0130_cloud import get_regist_duration_single_file 
os.environ['PYTHONIOENCODING'] = 'utf-8'

_MEMORY_PROCESSED_GLOBAL = False
sys.stdout.reconfigure(encoding='utf-8')
doc_details_path = None
# import io
# # 设置标准输出为 UTF-8 编码
...
def set_task_completed(task_id):
    # 定义URL
    url = 'http://localhost:8000/exec_task_success'
    ...
    try:
        ...
        if response.status_code == 200:
            logger.info('Sent request to set task status, and receive HTTP 200', response.text)
        else:
...
def set_task_failed(task_id, error_msg):
    # 定义URL
    url = 'http://localhost:8000/modify_task_error_status'
    ...
    def on_result(self, output_data_ptr):
        ...
        try:
            ...
            if job_id in self.pending_jobs and status in ["complete", "failed"]:
                ...
                # 只有在任务完成或失败时才移除 job_id 到 API 实例的映射
                if job_id in self.job_to_api_map:
...
class MultimodalAPI:
    ...
    def __init__(self, output_dir):
        ...
        self.registered_docs_file = self.output_dir / "registered_docs.json"
    ...
    def process_memory_data(self, min_wait_time=1.0, max_wait_time=10.0, enable_random_wait=True):
        ...
        memory_output_path = str(self.output_dir / f"Memory_data_with_responses_{timestamp}.xlsx"   )
        ...
        for row_index, row in df.iterrows():
            ...
            # 为每行创建新会话
            memory_api = MultimodalAPI(self.output_dir)
        ...
        if ocr_memory_result["success"]:
            ...
            # 保存OCR memory到JSON文件
            ocr_memory_json_path = memory_output_path.replace('.xlsx', '_get_all_ocr_memory.json')
                        ...
                        if isinstance(doc_list, list):
                            if len(doc_list) > 0:
                                ...
                                if not has_processing_docs and completion_rate >= 95:
                                    ...
                                    test_results_dir = self.output_dir / "testresult"
                                    test_results_dir.mkdir(exist_ok=True)
                                    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                                    default_excel_path = test_results_dir / f"document_registration_{timestamp}.xlsx"
    ...
    def save_document_details_to_excel(self, result_text, excel_path):
        ...
        try:
            ...
            if isinstance(doc_list, list) and len(doc_list) > 0:
                ...
                if doc_details:
                    # 修复路径处理逻辑
                    base_dir = os.path.dirname(excel_path) if excel_path else str(self.output_dir)
...
#############################################################################################################################################
    ...
    def process_document_registration_only(self):
        ...
        try:
            ...
            logger.info(f"文档注册分析 - doc_details_path: {doc_details_path}, quantum_core_path: {quantum_core_path}")
            if doc_details_path and quantum_core_path:
                logger.info("进行Quantum Core 日志注册分析")
                get_regist_duration_single_file(doc_details_path, quantum_core_path)
            else:
        ...
        for index in indices_to_process:
            ...
            try:
                ...
                if reuse_session:
                    ...
                    # 创建新的API实例
                    api = MultimodalAPI(self.output_dir)
...
def start_resource_monitoring(output_dir, gpu_type):
    """启动资源监控"""
    process_names =  ['LenovoQiraCore.exe','LenovoQiraCore.exe','mcpMgmtService.exe','mcpMgmtService.exe','lenovo.pfm.pipe.exe','file-index-service.exe','file-index-service.exe','file-search-service.exe','file-search-service.exe']# 需要监控的进程名
    ...
    log_file_path = logs_dir / f'resource_monitor_{timestamp}.log'
    resource_csv_path = resource_dir / f'resource_{timestamp}.csv'
    # 创建SystemMonitor实例
...
def run_quantum_e2e_process(args):
    ...
    try:
        ...
        if should_start_overall_monitor or should_start_combined_monitor:
            overall_monitor, overall_resource_csv_path, start_timestamp = start_resource_monitoring(args.output_dir, GPU_TYPE)
        elif should_start_pre_monitor and overall_resource_csv_path is None:
            # 为预处理阶段（memory和文档注册）启动监控
            pre_monitor, pre_resource_csv_path, start_timestamp = start_resource_monitoring(args.output_dir,GPU_TYPE)
        
        # 处理Memory数据（如果需要）
        memory_output_paths = []
        if args.process_memory or args.process_all:
            logger.info("Memory测试需要联网")
            control_wifi_on()
            # time.sleep(5)
            logger.info("开始处理Memory数据...")
            memory_api = MultimodalAPI(args.output_dir)
            ...
            if memory_success:
                ...
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
                memory_output_path = str(args.output_dir / f"Memory_data_with_responses_{timestamp}.xlsx")
        ...
        # 处理文档注册（如果需要）
        doc_output_paths = []
        if args.process_document or args.process_all:
            # 为文档注册创建新的API实例
            api = MultimodalAPI(args.output_dir)
        ...
        for excel_file in excel_files:
            ...
            try:
                # 为每个文件启动独立的资源监控
                file_monitor, file_resource_csv_path, _ = start_resource_monitoring(args.output_dir,GPU_TYPE)
                TEST_RESULTS_DIR = args.output_dir / "testresult"
                ...
                # 处理主Excel文件（如果需要）
                if success and (args.process_main or args.process_all):
...
def get_quantum_core_path():
    ...
    # 获取 C:\Users\[当前用户名]\AppData\Local\quantum_core 的路径
    # 动态获取当前用户目录，适用于不同设备的不同用户名
    
    # Returns:
    #     str: quantum_core 目录路径，如果路径不存在则返回 None
    try:
        # 获取当前用户目录
        user_home = Path.home()
...
class ConfigArgs:
    ...
    def __init__(self, config_dict, cmd_args):
        ...
        """
        # 获取optional配置
        optional = json.loads(config_dict.get('optional', '{}'))
        logger.info(f"Optional配置: {optional}")
        ...
        for f in os.listdir(SCRIPT_DIR):
            if os.path.isdir(f) and f.startswith("dataset"):
                EXCEL_DIR = SCRIPT_DIR / f
            if os.path.isdir(f) and f.startswith("Image"):
                IMAGE_DIR = SCRIPT_DIR / f
            if os.path.isdir(f) and f.startswith("Reg_Documents"):
                DOC_DIR = SCRIPT_DIR / f
            if f.startswith("Memory_data") and f.endswith(".xlsx"):
                MEMORY_EXCEL = SCRIPT_DIR / f
        # 如果有dataset_file_name

        self.excel_dir = str(EXCEL_DIR)
        
        # image_dir: 图片目录
        self.image_dir = str(IMAGE_DIR)
        
        # doc_dir: 文档目录
        self.doc_dir = str(DOC_DIR)
        
        # memory_excel: Memory Excel文件路径
        self.memory_excel = str(MEMORY_EXCEL)
        
        # =============================
        # 处理流程参数
        # =============================
        process_list = optional.get('process', ['Main'])
        if isinstance(process_list, str):
            process_list = [process_list]
        
        # 根据process列表设置各个process标志
        self.process_memory = 'Memory' in process_list
        self.process_document = 'Document' in process_list
        self.process_main = 'Main' in process_list
        self.process_all = 'All' in process_list
        
        # =============================
        # 测试参数
        # =============================
        # cycle: 循环次数
        cycle = optional.get('cycle', '1')
        self.cycle = int(cycle) if isinstance(cycle, str) else cycle
        
        # gpu_type: GPU类型
        node_type = config_dict.get('node_type', 'iGPU')
        gpu_type_map = {
            'dGPU': 'dGPU',
            'iGPU': 'iGPU',
            'aGPU': 'aGPU'
        }
        self.gpu_type = gpu_type_map.get(node_type, 'iGPU')
        
        # test_mode: 测试模式
        test_mode = optional.get('test_mode', 'local')[0]
        self.test_mode = test_mode if test_mode in ['cloud', 'local'] else 'local'
        
        # =============================
        # 命令行参数覆盖（优先级更高）
        # =============================
        # filter_local_brain: 命令行参数优先
        if hasattr(cmd_args, 'filter_local_brain') and cmd_args.filter_local_brain:
            self.filter_local_brain = cmd_args.filter_local_brain
        else:
            filter_lb = optional.get('filter_local_brain', 'no')
            if isinstance(filter_lb, bool):
                self.filter_local_brain = 'yes' if filter_lb else 'no'
            else:
                self.filter_local_brain = filter_lb if filter_lb in ['auto', 'yes', 'no'] else 'no'
        
        # min_wait_time: 命令行参数优先
        if hasattr(cmd_args, 'min_wait_time') and cmd_args.min_wait_time is not None:
            self.min_wait_time = float(cmd_args.min_wait_time)
        else:
            self.min_wait_time = float(optional.get('min_wait_time', 1.0))
        
        # max_wait_time: 命令行参数优先
        if hasattr(cmd_args, 'max_wait_time') and cmd_args.max_wait_time is not None:
            self.max_wait_time = float(cmd_args.max_wait_time)
        else:
            self.max_wait_time = float(optional.get('max_wait_time', 10.0))
        
        # enable_random_wait: 命令行参数优先
        if hasattr(cmd_args, 'enable_random_wait') and cmd_args.enable_random_wait is not None:
            self.enable_random_wait = bool(cmd_args.enable_random_wait)
        else:
            self.enable_random_wait = bool(optional.get('enable_random_wait', False))
        # cleanup_after_run: 命令行参数优先
        if hasattr(cmd_args, 'cleanup_after_run') and cmd_args.cleanup_after_run is not None:
            self.cleanup_after_run = bool(cmd_args.cleanup_after_run)
        else:
            self.cleanup_after_run = bool(optional.get('cleanup_after_run', False))

        # version_config / test_mode_config: 生成 config.json 时使用（命令行优先）
        self.version_config = getattr(cmd_args, 'version_config', 'v20260226_1120-R73.25')
        self.test_mode_config = getattr(cmd_args, 'test_mode_config', 'cloud')
        
        # =============================
        # 输出目录（命令行参数优先）
        # =============================
        if hasattr(cmd_args, 'output') and cmd_args.output:
            self.output_dir = Path(cmd_args.output)
        else:
            self.output_dir = SCRIPT_DIR / "output" / datetime.now().strftime("%Y%m%d_%H%M%S")
        ...
        # 额外信息（用于日志和回调）
        # =============================
        self.task_id = config_dict.get('task_id', None)
        self.test_object = config_dict.get('test_object', 'Quantum E2E')
        self.model_version_name = config_dict.get('model_version_name', 'v1016')
...
def load_config_and_create_args(config_path, cmd_args):
    """
    加载配置文件并结合命令行参数创建args对象
    ...
    try:
        ...
        # 创建args对象（配置文件 + 命令行参数）
        args = ConfigArgs(config_dict, cmd_args)
...
if __name__ == "__main__":
    ...
    # Get local path for "Desktop"
    user_profile = os.environ.get('USERPROFILE')
    if user_profile:
        desktop_path = os.path.join(user_profile, "Desktop")
        llm_eval_path = os.path.join(desktop_path, "LRIA_llm_eval")
        logger.info(llm_eval_path)
    else:
        logger.info("Failed to get user's desktop path")

    # 创建 ArgumentParser 对象
    parser = argparse.ArgumentParser(description='...')

    # 添加参数   
    parser.add_argument('-c', '--config_path', default="quantum_e2e_test.json", help='config file path')
    parser.add_argument('-o', '--output',  default="output", help='output file path')
    
    
    
    parser.add_argument('--filter_local_brain', type=str, default="no", 
                       choices=["auto", "yes", "no"],
                       help='是否过滤Local Brain数据')
    # 等待时间配置
    ...
    # 20260226 更新：新增参数控制生成 config.json
    parser.add_argument('--version_config', type=str, default="v20260226_1120-R73.25", help='生成config.json时使用的version字段')
    parser.add_argument('--test_mode_config', type=str, default="cloud", choices=["cloud", "local"], help='生成config.json时使用的test_mode字段')
    ...
    try:
        logger.info(f"使用配置文件: {cmd_args.config_path}")
        
        args = load_config_and_create_args(cmd_args.config_path, cmd_args)
        ...
        # 读取 C:\config.json 中的 device_type 和 device_model，并在输出目录下生成新的 config.json
        try:
            ...
            device_type = original_config.get("device_type")
            device_model = original_config.get("device_model")

            new_config = {
                "version": getattr(args, "version_config", "v20260226_1120-R73.25"),
                "test_mode": getattr(args, "test_mode_config", "cloud"),
            }
            if device_type is not None:
                new_config["device_type"] = device_type
            if device_model is not None:
                new_config["device_model"] = device_model
        ...
        if success:
            ...
            if (
                (args.process_main or args.process_all)
                and str(getattr(args, "test_mode_config", getattr(args, "test_mode", "cloud"))).lower() == "cloud"
            ):
                logger.info("处理在日志中获取memory和kbqa的Retrieval结果")
                quantum_core_path = get_quantum_core_path()
                if quantum_core_path:
                    for output_file in output_files:
                        log_directory = quantum_core_path
                        if not os.path.exists(log_directory):
                            logger.warning(f"日志目录不存在: {log_directory}")
                            continue
                    
                        if not os.path.exists(output_file):
                            logger.warning(f"Excel文件不存在: {output_file}")
                            continue
                        ...
                        try:
                            process_excel(
                                ...
                                output_path=output_result_file,
                                scene_filter=None  # 不过滤场景
                            )
                            logger.info(f"成功处理Excel文件: {output_file}")
                        except Exception as e:
                            logger.error(f"处理Excel文件时出错: {e}")
            # 任务成功
            set_task_completed(args.task_id)
            # if args.process_main or args.process_all:
            #     if args.cleanup_after_run:
            #         logger.info("根据参数清理所有数据...")
            #         try:
            #             api_instance = MultimodalAPI(args.output_dir)
            ...
            # 日志打包输出至本次运行目录
            logger.info("开始打包日志...")
            packageLog.main(args.output_dir)
            try:
                ...
                if zip_candidates:
                    non_target_candidates = [
                        ...
                        and not p.name.startswith("TestResult_")
                    ]
                    if not non_target_candidates:
                        if target_zip.exists():
                            logger.info("日志压缩包已为 PackageAllLogs.zip，跳过重命名")
                        else:
                            logger.warning("未找到可用于重命名为 PackageAllLogs.zip 的日志压缩包")
                    else:
                        zip_to_rename = max(non_target_candidates, key=lambda p: p.stat().st_mtime)
                        zip_to_rename.replace(target_zip)
            except Exception as e:
                logger.error(f"重命名日志压缩包失败: {e}")
            logger.info("日志打包完成")

            # local 模式下，在打包日志后调用本地日志解析脚本
            try:
                if (
                    str(getattr(args, "test_mode_config", getattr(args, "test_mode", "cloud"))).lower() == "local"
                    and (args.process_main or args.process_all)
                ):
                    logger.info("检测到 test_mode_config=local，开始执行 analysis_log_local 日志解析...")
                    try:
                        analysis_log_local.main(str(args.output_dir))
                        logger.info("analysis_log_local 执行完成")
                    except SystemExit as e:
                        logger.error(f"analysis_log_local 触发 SystemExit，已捕获避免退出主程序: {e}")
            except Exception as e:
                logger.error(f"调用 analysis_log_local 失败: {e}")
            if args.process_main or args.process_all:
            # 将除 PackageAllLogs.zip 外的所有内容打包为一个总包 zip（含版本、模式与时间戳）
                try:
                    mode_for_bundle = str(getattr(args, "test_mode_config", getattr(args, "test_mode", "cloud"))).lower()
                    version_for_bundle = str(getattr(args, "version_config", "")).strip()
            ...
            logger.info("流程执行失败！")
            if hasattr(args, 'test_mode') and args.test_mode == "local":
                logger.info("检测到本地测试模式，上传结果前打开WiFi...")
                control_wifi_on()
            set_task_failed(args.task_id,"流程执行失败")
    except Exception as e:
        logger.error(f"主流程执行过程中出现错误: {e}")
        if hasattr(args, 'test_mode') and args.test_mode == "local":
            logger.info("检测到本地测试模式，上传结果前打开WiFi...")
            control_wifi_on()
        set_task_failed(args.task_id,e)
    finally:
        # Clean up quantum client
        logger.info("清理量子客户端资源...")
        quantum_client_manager.cleanup()
        if hasattr(args, 'test_mode') and args.test_mode == "local":
            logger.info("执行完成，确保打开WiFi...")
        set_task_completed(args.task_id)
        # input("请按任意键结束程序...")  
