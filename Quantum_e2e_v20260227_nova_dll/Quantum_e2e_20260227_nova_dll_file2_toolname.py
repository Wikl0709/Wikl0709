...
from utils.getDeviceInfo import getDeviceInfo  # 依赖 wmi，未安装则直接报错
import atexit
import sys
import random
import shutil
...
# 运行上下文（由 __main__ 在解析 args 后初始化）
RUN_ID = None
OUTPUT_ROOT = None
LOGS_DIR = None
RESOURCE_DIR = None
...
def init_run_context(version_config: str):
    """
    初始化本次运行的输出目录结构。
    要求：RUN_ID 使用 version_config 的值（用于输出目录名）。
    """
...
def create_output_bundle_zip(
    ...
    # 统一测试结果压缩包命名：TestResult_{version}_{mode}_{timestamp}.zip（version 为空时不拼）
    if ver:
...
class MultimodalAPI:
    ...
    def __init__(self):
        ...
        self.registered_docs_file = str(REGISTERED_DOCS_PATH)
    ...
    def process_memory_data(self, min_wait_time=1.0, max_wait_time=10.0, enable_random_wait=True):
        ...
        # Memory 输出统一写入本次运行目录下
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        memory_output_path = OUTPUT_ROOT / f"Memory_data_with_responses_{timestamp}.xlsx"
        # 记录到实例上，供外部使用
        ...
        try:
            ...
            if isinstance(doc_list, list) and len(doc_list) > 0:
                ...
                if doc_details:
                    ...
                    base_dir = TEST_RESULTS_DIR
                    base_dir.mkdir(parents=True, exist_ok=True)
                    doc_details_path = os.path.join(str(base_dir), f"{base_name}_document.xlsx")
...
def start_resource_monitoring(gpu_type):
    ...
    # 日志与资源监控统一写入本次运行目录
    log_file_path = str(LOGS_DIR / f'resource_monitor_{timestamp}.log')
    resource_csv_path = str(RESOURCE_DIR / f'resource_{timestamp}.csv')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--image_dir', type=str, default=r"C:\Users\woel0\Desktop\LRIA_llm_eval\eval\Quantum_e2e_v20260203_nova_dll\dataset\EN_memory_v8\Image_v9_260210", help='是否需要用到pathlist')
    parser.add_argument('--doc_dir', type=str, default=r"C:\Users\woel0\Desktop\LRIA_llm_eval\eval\Quantum_e2e_v20260203_nova_dll\dataset\EN_memory_v8\EN_KBQA", help='文档注册,文档的路径')
    parser.add_argument('--memory_excel', type=str, default=r"C:\Users\woel0\Desktop\LRIA_llm_eval\eval\Quantum_e2e_v20260203_nova_dll\dataset\EN_memory_v8\Memory_data _v9_260201_en.xlsx", help='Memory注册,Excel文件路径')
    ...
    # === 初始化本次运行上下文 ===
    # RUN_ID 需使用 main 入口传入的 version_config 值
    init_run_context(args.version_config)
    setup_logging(OUTPUT_ROOT)

    logger.info("程序启动参数: " + ", ".join([f"{k}={v}" for k, v in vars(args).items()]))

    # 读取 C:\config.json 中的 device_type 和 device_model，并在 OUTPUT_ROOT 下生成新的 config.json
    try:
        ...
        # 调用 getDeviceInfo 获取本机设备信息并合并到 config（依赖 wmi，失败则直接报错）
        ...
        raise  # 无 wmi 或 getDeviceInfo 失败时不继续后续操作，直接报错退出
    ...
    try:
        ...
        if success:
            ...
            # 根据参数处理在日志中获取memory和kbqa的Retrieval结果
            # 仅 cloud 模式走当前逻辑；local 模式改为打包日志后调用 analysis_log_local 解析
            if (
                (args.process_main or args.process_all)
                and str(getattr(args, "test_mode_config", "cloud")).lower() == "cloud"
            ):
        ...
        try:
            # 将打包结果统一命名为 PackageAllLogs.zip
            output_root_path = Path(OUTPUT_ROOT)
            target_zip = output_root_path / "PackageAllLogs.zip"

            # 只从当前目录中选择“日志压缩包”来重命名，显式排除测试结果压缩包 TestResult_*.zip，
            # 避免错误地把测试结果包改名为 PackageAllLogs.zip。
        ...
        # 20260227更新
        # local 模式下，且需要处理main/全流程时，在打包日志后调用本地日志解析脚本
        try:
            if (
                str(getattr(args, "test_mode_config", "cloud")).lower() == "local"
                ...
                try:
                    ...
                    # analysis_log_local 内部可能会 sys.exit()，这里捕获避免中断主流程
                    logger.error(f"analysis_log_local 触发 SystemExit，已捕获避免退出主程序: {e}")
        except Exception as e:
            logger.error(f"调用 analysis_log_local 失败: {e}")

        # 仅当 process_main 或 process_all 时：将除 PackageAllLogs.zip 外的所有内容打包为一个总包 zip（含版本、模式与时间戳）
        if args.process_main or args.process_all:
            try:
                mode_for_bundle = str(getattr(args, "test_mode_config", "cloud")).lower()
