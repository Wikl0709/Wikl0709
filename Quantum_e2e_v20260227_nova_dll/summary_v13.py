...
def _has_nonempty_col(df: pd.DataFrame, col: str) -> bool:
    """列存在且至少有一个非空值"""
    return (col in df.columns) and df[col].apply(lambda x: not _is_empty(x)).any()
...
# -------------------- Time Stats Post-process --------------------

def _fill_when_all_zero(df: pd.DataFrame, row_mask: pd.Series, target_cols: list[str], source_cols: list[str]) -> pd.DataFrame:
    """Generic helper: for rows where row_mask is True, if *all* target_cols are 0 (or NaN -> 0),
    copy values from source_cols into target_cols. Returns a new DataFrame (does not mutate input).

    Notes:
    - Non-numeric values are coerced to NaN, then treated as 0 for the all-zero check.
    - Length of target_cols must equal length of source_cols.
    """
    out = df.copy()
    if not target_cols or len(target_cols) != len(source_cols):
        return out
    # Coerce to numeric for the all-zero test
    numeric_targets = [pd.to_numeric(out[c], errors='coerce').fillna(0) if c in out.columns else pd.Series([0]*len(out)) for c in target_cols]
    all_zero = row_mask.copy()
    for s in numeric_targets:
        all_zero = all_zero & s.eq(0)
    if all_zero.any():
        out.loc[all_zero, target_cols] = out.loc[all_zero, source_cols].values
    return out


def postprocess_time_stats_v2(df: pd.DataFrame) -> pd.DataFrame:
    """Post-process the timestatv2 sheet for special fallback rules.

    Rule 1: For rows where domain == 'file search', if ttft, ttft_tp95, ttft_tp50 are all zeros, 
            then fill them from response_time, response_time_tp95, response_time_tp50 respectively.
    """
    if df is None or len(df) == 0:
        return df
    required = {"domain","ttft","ttft_tp95","ttft_tp50","response_time","response_time_tp95","response_time_tp50"}
    if not required.issubset(df.columns):
        return df
    dnorm = df["domain"].apply(_norm_str)
    mask = dnorm == "file search"
    df2 = _fill_when_all_zero(
        df,
        row_mask=mask,
        target_cols=["ttft","ttft_tp95","ttft_tp50"],
        source_cols=["response_time","response_time_tp95","response_time_tp50"],
    )
    return df2
...
# -------------------- Multi-turn Pass Rate (by domain) --------------------
def build_multi_turn_pass_rate(df: pd.DataFrame) -> pd.DataFrame:
    """
    在 df_clean 上计算多轮通过率：
    - 以 session_id 相同为一轮；
    - 一轮中所有样本 Result ∈ {1, pass} 记为 pass；否则（含 fail/0/空/其它值）记为 fail；
    - 以 domain 聚合 -> 输出两列：domain, multi_turn_pass_rate (x/y = z%).
    """
    if not _has_nonempty_col(df, SESSION_ID_COL):
        return pd.DataFrame(columns=["domain", "multi_turn_pass_rate"])

    # 规范 domain/secondary；只保留 session_id 非空
    df2 = ensure_norm_domain_secondary(df)
    df2 = df2.copy()
    df2["_domain_norm"] = df2[DOMAIN_COL].apply(_norm_str)
    df2["_result_norm"] = df2[FINAL_ANSWER_COL].apply(_norm_str)
    df2 = df2[~df2[SESSION_ID_COL].apply(_is_empty)]

    if len(df2) == 0:
        return pd.DataFrame(columns=["domain", "multi_turn_pass_rate"])

    pass_like = {"1", "pass"}  # 按需求仅 1/pass 记为通过

    rows = []
    for dom, g_dom in df2.groupby("_domain_norm", dropna=False):
        # 对每个 domain，再按 session_id 分组判定一轮是否通过
        passed_sessions = 0
        total_sessions = 0
        for sid, g_sess in g_dom.groupby(SESSION_ID_COL, dropna=False):
            total_sessions += 1
            # 所有样本都在 pass_like 且至少有一条样本
            r = g_sess["_result_norm"]
            sess_pass = len(r) > 0 and r.isin(pass_like).all()
            if sess_pass:
                passed_sessions += 1

        rows.append({
            "domain": dom,
            "multi_turn_pass_rate": _fmt_rate(passed_sessions, total_sessions)
        })
...
def summarize(
    ...
    time_stats_v2_df = postprocess_time_stats_v2(time_stats_v2_df)
    ...
    # 8.5) NEW: multi-turn（存在且有值的 session_id 时才计算）
    multi_turn_df = None
    if _has_nonempty_col(df_clean, SESSION_ID_COL):
        multi_turn_df = build_multi_turn_pass_rate(df_clean)
    ...
    with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
        ...
        # NEW: 输出 multi-turn sheet（仅当有数据时）
        if multi_turn_df is not None and len(multi_turn_df) > 0:
            multi_turn_df.to_excel(writer, index=False, sheet_name="multi-turn")
...
if __name__ == "__main__":
    ...
    in_path = r"D:\downloads\test_results_local_solution_case_EN_0210_20260302_162617_cycle1_Auto_Judge_Final.xlsx"
    out_path = r"D:\downloads\test_results_local_solution_case_EN_0210_20260302_162617_cycle1_Auto_Judge_Final_sum1.xlsx"
    sheet_name = 0
