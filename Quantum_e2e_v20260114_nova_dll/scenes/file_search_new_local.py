...
def type_id_check(result_text):
    ...
    content = content_dict["content"][0]
    text_str=content['text']
    text_str=json.loads(text_str)
    total_text = text_str["total_count"]
    result_str=text_str['results']
...
def main(row: Dict):
    ...
    id_num = row.get('label_judge', '').strip().lower()
    id_num=str(id_num)
    ...
    pat = re.compile(r'"message"\s*:\s*"([^"]*)"(?=\s*,\s*"timestamp")')
    try:
        message = pat.search(result_text).group(1)
    except:
        message = 'error'
    print(message)
    if str(result_text) == 'CTTVError: Empty response from server':
        ...
        try:
            ...
            pat = re.compile(r'"file_name":"(.*?)","file_path"', re.DOTALL)
